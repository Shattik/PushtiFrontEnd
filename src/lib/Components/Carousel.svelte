...
const canChangeSlide = ({
  ...
  // Carousel
  let divClass: string = 'grid overflow-hidden static rounded-lg  sm:h-64 xl:h-80 2xl:h-96';
...
<div bind:this={carouselDiv}  on:mousedown|nonpassive={onDragStart} on:touchstart|nonpassive={onDragStart} on:mousemove={onDragMove} on:mouseup={onDragStop} on:touchmove={onDragMove} on:touchend={onDragStop} role="button" aria-label={ariaLabel} tabindex="0">
