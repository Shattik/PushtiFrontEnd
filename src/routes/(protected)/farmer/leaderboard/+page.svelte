<script>
    ...
    let data = $page.data;
    let farmerLeaderboard = data.leaderboard;
    let selfRank = data.selfRank[0];
    let farmerIndex = farmerLeaderboard.findIndex((x) => x.nid == selfRank.nid);
    console.log(selfRank);
    console.log(selfRank.nid == farmerLeaderboard[0].nid);
  </script>
  ...
  <div class="flex flex-row">
    ...
    <div class="w-full h-screen p-5">
      ...
      <Table shadow>
        ...
        {:else}
          {#if pagination_page_farmer * 10 > farmerIndex}
            <TableBodyRow
            ...
            >
              <TableBodyCell class="text-custom_font-table-header text-center">
                {#if farmerLeaderboard[farmerIndex].rank == 1}
                  <MedalSolid class="w-5 h-5 text-rank-gold m-auto" />
                {:else if farmerLeaderboard[farmerIndex].rank == 2}
                  <MedalSolid class="w-5 h-5 text-rank-silver m-auto" />
                {:else if farmerLeaderboard[farmerIndex].rank == 3}
                  <MedalSolid class="w-5 h-5 text-rank-bronze m-auto" />
                {:else}
                  {farmerLeaderboard[farmerIndex].rank}
                {/if}
              </TableBodyCell>
              <TableBodyCell
              ...
              >
                <Avatar
                  class="w-7 h-7 me-2"
                  src={farmerLeaderboard[farmerIndex].avatarLink}
                  alt="avatar"
                />
                {farmerLeaderboard[farmerIndex].name}
              </TableBodyCell>
              <TableBodyCell class="text-custom_font-table-header text-center"
                >{farmerLeaderboard[farmerIndex].points}</TableBodyCell
              >
            </TableBodyRow>
            {#if pagination_page_farmer * 10 != farmerIndex + 1}
              <TableBodyRow
            ...
            {/if}
          {/if}
          {#each farmerLeaderboard.slice(pagination_page_farmer * 10, (pagination_page_farmer + 1) * 10 + 1) as { name, points, rank, avatarLink }, i}
            ...
            {/if}
          {/each}
          {#if pagination_page_farmer * 10 + 10 <= farmerIndex}
            {#if pagination_page_farmer * 10 + 10 != farmerIndex}
              <TableBodyRow
                class="border-b-2 border-divider_col bg-body_custom drop-shadow-md"
              >
            ...
            >
              ...
              <TableBodyCell class="text-custom_font-table-header "
                >{farmerLeaderboard[farmerIndex].name}</TableBodyCell
              >
              <TableBodyCell
                class="text-custom_font-table-header flex flex-row items-center"
              >
                <Avatar
                  ...
                  alt="avatar"
                />
                {farmerLeaderboard[farmerIndex].agentname}
              </TableBodyCell>
              <TableBodyCell class="text-custom_font-table-header text-center"
                >{farmerLeaderboard[farmerIndex].points}</TableBodyCell
              >
