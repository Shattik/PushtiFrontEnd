  ...
  <div class="flex flex-row">
              ...
              <Table divClass="grow relative overflow-x-auto">
                  <TableBody>
                      <TableBodyRow class="border-b-2 border-divider_col bg-body_custom drop-shadow-md">
                          ...
                          <TableBodyCell class="text-custom_font-light">{vendor.name}</TableBodyCell>
                      </TableBodyRow>
                      <TableBodyRow class="border-b-2 border-divider_col bg-body_custom drop-shadow-md">
                          <TableBodyCell class="text-custom_font-deep font-bold">NID</TableBodyCell>
                          <TableBodyCell class="text-custom_font-light">{vendor.nid}</TableBodyCell>
                      </TableBodyRow>
                      <TableBodyRow class="border-b-2 border-divider_col bg-body_custom drop-shadow-md">
                          <TableBodyCell class="text-custom_font-deep font-bold">Mobile</TableBodyCell>
                          <TableBodyCell class="text-custom_font-light">{vendor.phone}</TableBodyCell>
                      </TableBodyRow>
                      <TableBodyRow class="border-b-2 border-divider_col bg-body_custom drop-shadow-md">
                          <TableBodyCell class="text-custom_font-deep font-bold">Agent</TableBodyCell>
                          <TableBodyCell class="text-custom_font-light">{vendor.agent}</TableBodyCell>
                      </TableBodyRow>
                  </TableBody>
              </Table>
          </div>
      ...
      <Card bind:this={card} class="max-w-full w-full bg-body_custom">
          <div class="flex flex-row justify-between items-center mb-3">
              ...
              <h1 class="text-xl text-custom_font-deep font-bold ms-1">{vendor.points} Points</h1>
          </div>
          <Progressbar {progress} animate tweenDuration={1500} easing={sineOut} size="h-3"  class="mb-3 bg-primary-200" labelInside labelInsideClass="h-3 text-transparent text-xs font-medium text-center p-0.5 leading-none rounded-full bg-rank-{vendor.rank.toLowerCase()}"/>
          <p class="text-lg text-custom_font-deep font-medium">{vendor.more_points} points to <span class={twJoin("text-lg font-medium", nextRankColor)}>{vendor.next_rank}</span></p>
          <p class="my-3"><span class={twJoin("text-xl font-medium", textColor)}>Your Rewards</span></p>
          {#each vendor.benefits as benefit}
          <div class="flex flex-row">
              <StarRegular class="w-6 h-6 me-3 mb-4 text-rank-{vendor.rank.toLowerCase()}"/>
