<script>
  ...
  <div role="navigation" on:mouseenter={()=>{focused=true;}} on:mouseleave={()=>{focused=false;}} aria-label="placeholder" in:fly={{x:-50, y:0, duration:500}}>
  <Sidebar class="w-64 h-screen fixed bg-sidebar_bg overflow-x-hidden" 
    ...
    <SidebarWrapper class="overflow-x-hidden h-screen fixed w-64 bg-sidebar_bg">
      <SidebarGroup class="w-48">
        ...
        <SidebarItem label="Dashboard" href="/admin/dashboard" {spanClass}>
        ...
        <SidebarItem label="Reports" href="/admin/report" {spanClass}>
          <svelte:fragment slot="icon">
            <FileContractSolid class="w-7 h-7 ms-1.5 me-3 text-logo-1 transition duration-75 dark:text-gray-400 group-hover:text-logo-2 dark:group-hover:text-white" />
          </svelte:fragment>
        </SidebarItem>
        <SidebarItem  label = "Product Management" href="/admin/product-management" {spanClass}>
          <svelte:fragment slot="icon">
        ...
        <SidebarItem label="Tickets" href="/admin/support" {spanClass}>
          <svelte:fragment slot="icon">
