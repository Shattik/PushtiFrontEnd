<script>
  ...
  let more_points =
    parseInt(data.rankandpoint.maxPoint) -
    parseInt(data.rankandpoint.points) +
    1;
  let farmer = {
  ...
  onMount(async () => {
    progress = (farmer.points / rank_point_range) * 100;
...
<div class="flex flex-row">
  ...
  <div class="w-full h-screen p-5">
    ...
    <Card class="max-w-full w-full bg-body_custom mb-5 h-[32rem]">
      <p class="text-2xl font-bold text-custom_font-deep mb-5">
        Monthly Transactions
      </p>
      <Scatter
        labels={salesDataMonthList}
        data={salesDataAmountList}
