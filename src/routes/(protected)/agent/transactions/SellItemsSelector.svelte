<script>
    ...
    function addItem(index){
        let item = {
            ...
            tax: 0,
            amount: filteredItems[index].amount,
...
<div class="grid grid-cols-6 gap-3 place-items-stretch">
    ...
    <div class="text-sm text-custom_font-deep font-bold text-right">Max Amount</div>
    ...
    {#if selectedItems.length > 0}
        {#each selectedItems as item,index}
            ...
            <input type="number" class="w-full p-2 border-2 border-divider_col rounded-md text-sm text-right" bind:value={item.quantity} on:input={()=>{item.quantity = Math.min(Math.max(item.quantity,0),item.amount);item.total=item.quantity*item.price; item.tax = Math.round(item.total*item.taxPercentage/100);}} on:keydown={handleNumKey} autofocus required/>
            <input class="w-full p-2 border-2 border-divider_col rounded-md text-sm text-right" value={`${item.amount} ${item.unit}`} disabled/>
